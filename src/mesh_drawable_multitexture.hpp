...
struct mesh_drawable_multitexture : vcl::mesh_drawable {
	...
	GLuint texture_3; // add a third texture
	GLuint texture_4; // add a third texture
};
...
void draw(mesh_drawable_multitexture const& drawable, SCENE const& scene)
{
	...
	vcl::opengl_uniform(drawable.shader, "image_texture_3", 2);  opengl_check; // the third texture is called "image_texture_3" in the shader in this case

	glActiveTexture(GL_TEXTURE3); opengl_check; // the additional texture 
	glBindTexture(GL_TEXTURE_2D, drawable.texture_4); opengl_check;
	vcl::opengl_uniform(drawable.shader, "image_texture_4", 3);  opengl_check; // the third texture is called "image_texture_3" in the shader in this case
